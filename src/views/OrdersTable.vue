    ...
    >
      ...
      <template #item="{ item }">
        <tr>
          <!-- 展开/收起按钮列 -->
          <td>
            <v-icon @click="toggleExpand(item)" class="cursor-pointer">
        ...
        <!-- 渲染子级行 -->
        <tr
          v-if="isExpanded(item) && item.children && item.children.length > 0"
        >
          <td colspan="10">
            <v-data-table
      ...
      return this.orders.filter((order) => {
        ...
        if (this.filterUnposted === "hasUnposted") {
          unpostedMatch =
            ...
            order.children.some((child) => !child.gztime);
        } else if (this.filterUnposted === "noUnposted") {
          unpostedMatch =
            order.children &&
            order.children.length > 0 &&
            order.children.every((child) => child.gztime);
        }
        ...
        if (this.filterCompletion === "above90") {
          completionMatch =
            order.pcs > 0 && (order.getpcs / order.pcs) * 100 >= 90;
